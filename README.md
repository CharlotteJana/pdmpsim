...
used as title for the plot methods. Slot **dynfunc** returns the ODEs of
the variables, their value depends on the state of the discrete variable
`d`. Slot **ratefunc** determines the rate and therefore probability
...
function takes the model as argument and returns the same model, with
...
reproducible. The vector `seeds` and the PDMP model are the only
...
Most of them are generated with package `ggplot2` and can be modified
afterwards.

## Plot single simulations

The trajectory of a single simulation can be plotted with function
`plot`, if the simulation is stored in slot `out` of the model. The
...
another function is necessary. It is called `plotSeeds` and can plot up
to four different trajectories at a
time.
...
Function `plotTimes` allows to plot a box plot or violin plot for
different time values over all simulations. Argument `plottype`
...
A special argument to function `plotTimes` is called `nolo` which stands
for *number of labelled outliers*. This allows to label the most extreme
outliers with the seed number which was used to create the simulation.
If for example `nolo = 3`, a maximum of three outliers will be labelled
for every time
value.
...
Function `hist` plots a histogram over all simulations for a given time
value. Function `density` creates a density plot, allowing multiple time
values as
input.
...
Use function `summarise_at` to apply functions as `mean`, `sd`, etc. on
the simulated values. Function `plotStats` calls `summarise_at`
...
Package `pmdpsim` provides the function `generator` to numerically
...
Because the generator operates on functions, it hast to be applied to a
