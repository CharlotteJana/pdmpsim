#======== todo =================================================================
#t hist: bei mehreren diskreten Variablen wird der Barplot nicht 
#t       korrekt beschriftet (names.arg = c("dA", "dB") geht nicht!)
#t plotStats: label oben hinzufügen
#t plotStats: missing funktioniert nicht, warum?
#t summarize_at: correct generic function definition?
#t density: in plotDensity umbenennen?
#t density: warum muss stats in imports und darf nicht zu suggest?
...
#' 
#' @param x data.frame of class \code{\link{multSimData}}.
#' @param vars character vector giving the names of variables to plot the statistics for.
#' @param funs a list of function calls generated by \code{\link[dplyr]{funs}}, 
#' or a character vector of function names, or simply a function.
#' @note The method requires the packages \pkg{tidyr}, \pkg{grid} and \pkg{gtable}.
...
#' All outliers that outreach a given threshold will be printed with a
...
#' @examples 
#' data("simplePdmp")
#' md <- getMultSimData(multSim(simplePdmp, 1:20), times = 1:5)
